    ...
    public class Plugin : IDalamudPlugin
    {
        ...
        public Plugin()
        {
            ...
            this.commandManager.AddHandler(
                ...
                {
                    HelpMessage = "Opens the Shears settings menu.",
                    ShowInHelp = true
                }
            );
        ...
        private IntPtr CharacterInitializeDetour(IntPtr drawObjectBase, IntPtr customizeDataPtr)
        {
            if (lastWasPlayer)
            {
                ...
                if (actor != null &&
                    ...
                    && this.clientState.LocalPlayer != null
                    && actor.ObjectId == this.clientState.LocalPlayer.ObjectId
                    && this.config.ChangeSelf)
                {
                    lastWasSelf = true;
                    this.ChangeRace(customizeDataPtr, this.config.ChangeSelfTargetRace);
                }
            }
        ...
        private void ChangeRace(IntPtr customizeDataPtr, Race targetRace)
        {
            ...
            if (this.config.OnlyChangeHrothgars && lastWasSelf == false)
            {
                hrothBool = customData.Race == Race.HROTHGAR;
            }
            else
            {
                hrothBool = customData.Race != targetRace;
            }
            lastWasSelf = false;
        ...
        public void OnlyChangeHrothgars(bool onlyChangeHrothgars)
        {
            if (this.config.OnlyChangeHrothgars == onlyChangeHrothgars)
            {
                return;
            }

            PluginLog.Log($"Only change hrothgar players toggled to {onlyChangeHrothgars}, refreshing players");
            this.config.OnlyChangeHrothgars = onlyChangeHrothgars;
        ...
        {
            if (this.config.ChangeSelfTargetRace == race)
            {
                return;
            }

            PluginLog.Log($"Target race for player changed to {race}, refreshing players");
            this.config.ChangeSelfTargetRace = race;
            unsavedConfigChanges = true;
        }
        ...
        protected virtual void Dispose(bool disposing)
        {
            ...
            this.commandManager.RemoveHandler("/shears");
